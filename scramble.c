...
/**
* Converts the string to its upper-case
*/
string to_upper(string str)
{
...
int binary_search(string s, word words[], int min, int max)
{
    if (min > max)
        return -1;
    ...
    if (strcmp(s, words[mid_index].letters) < 0)
        return binary_search(s, words, min, mid_index - 1);
    else if (strcmp(s, words[mid_index].letters) > 0)
        return binary_search(s, words, mid_index + 1, max);
    else
        return mid_index;
}
...
bool lookup(string s)
{
    ...
    int found_index = binary_search(s, dictionary.words, 0, dictionary.size);
    
    // See if we found this word already
    if (found_index >= 0)
        if (!dictionary.words[found_index].found)
        {
            dictionary.words[found_index].found = true;
            return true;
        }
...
void scramble(void)
{
    // Make a copy of the grid here
    char grid_scrambled[DIMENSION][DIMENSION];
    
    // Scramble the original grid
    for (int row = 0; row < DIMENSION; row++)
        for(int col = 0; col < DIMENSION; col++)
            // Calculate the transfomed position of the symbol
            grid_scrambled[col][DIMENSION - 1 - row] = grid[row][col];
            
    // Copy scrambled grid to original
    for (int row = 0; row < DIMENSION; row++)
        for(int col = 0; col < DIMENSION; col++)
